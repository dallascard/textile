...
def main():
    ...
    p_y_given_x = sample_predictions(X_test, w, V_n, add_intercept=True, n_samples=50)
    print(p_y_given_x.mean(axis=1))
...
def sample_predictions(X, w_n, V_n, add_intercept=False, n_samples=20):
    """return (n_items, n_samples) probs """
    if add_intercept:
        X = add_ones_col(X)
