...
def main():
    ...
    plot_over_time(project_dir, subset, label, model_type, penalty, dh, sample_labels, suffix)


def plot_over_time(project_dir, subset, label, model_type, penalty, dh, sample_labels, suffix):
    ...
    files = glob.glob(os.path.join(dirs.dir_models(project_dir), model_basename + '_????_fight', 'results.csv'))
    print(files)
