...
def predict_expanded_model(project_dir, model, model_name, test_subset, label_name, items_to_use=None, verbose=False, force_dense=False):


    X, features_concat = load_data_expanded_model(project_dir, model_name, test_subset, items_to_use, force_dense, verbose)
    print("Loaded test data: (%d, %d)" % X.shape)

    coefs = model.get_coef_array()
    n_terms_old, n_classes = coefs.shape
    old_terms = model._col_names
    old_index = dict(zip(old_terms, np.arange(len(old_terms))))
    new_terms = features_concat.terms
    n_terms = len(new_terms)
    new_index = dict(zip(old_terms, np.arange(n_terms)))
    print("Creating new coefficient matrix")
    print(n_terms, n_classes)
    new_coefs = np.zeros((n_terms, n_classes))
    for term in old_terms:
        new_coefs[new_index[term], :] = coefs[old_index[term], :]
...
def load_data_expanded_model(project_dir, model_name, test_subset, items_to_use=None, force_dense=False, verbose=False):
    model_dir = os.path.join(dirs.dir_models(project_dir), model_name)
    feature_signatures = fh.read_json(os.path.join(model_dir, 'features.json'))
    test_features_dir = dirs.dir_features(project_dir, test_subset)
    ...
    for sig in feature_signatures:
        ...
        if items_to_use is not None:
            ...
            test_feature = features.create_from_feature(test_feature, indices_to_use)
            printv("New shape = (%d, %d)" % test_feature.get_shape(), verbose)
        test_feature.threshold(feature_def.min_df)
        printv("Setting vocabulary", verbose)
        test_feature.expand_terms(sig['terms'])
        old_terms_list.extend(sig['terms'])
        idf = None
        if feature_def.transform == 'tfidf':
            idf = sig['idf']
        word_vectors_prefix = sig['word_vectors_prefix']
    ...
    new_indices = []
    old_terms_set = set(old_terms_list)
    for t_i, term in enumerate(features_concat.get_terms()):
        if term not in old_terms_set:
            new_indices.append(t_i)
    print("%d new terms" % len(new_indices))

    return X, features_concat, new_indices
